-- FIX: Remove Function Overloading
-- Your screenshot shows TWO versions of 'submit_waste_report'. 
-- This is why it's still going to the wrong branch—the old code is still there!

-- 1. Drop both versions specifically
DROP FUNCTION IF EXISTS public.submit_waste_report(uuid, numeric, text, text);
DROP FUNCTION IF EXISTS public.submit_waste_report(uuid, uuid, numeric, text, text);

-- 2. Recreate ONLY the correct branch-aware version
...
BEGIN
    ...
    INSERT INTO public.waste_logs (branch_id, ingredient_id, reported_by, waste_reason, quantity, unit_type, cost_snapshot, notes) 
    VALUES (p_branch_id, p_ingredient_id, v_user_id, p_reason, p_quantity, COALESCE(v_unit_type, 'unit'), v_total_cost, p_notes) 
