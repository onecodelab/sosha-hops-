-- Fix chatbot order attribution so chatbot orders are not stamped with a human waiter.
-- Also backfill existing chatbot orders so receipts and admin views stop showing a fake staff owner.
...
BEGIN
    ...
    v_waiter_id := CASE
        WHEN v_source = 'chatbot' THEN NULL
        ELSE COALESCE((p_order_details->>'waiter_id')::UUID, v_user_id)
    END;
