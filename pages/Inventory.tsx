...
import { 
  Search, AlertTriangle, Package, Truck, 
...
const Inventory: React.FC = () => {
  ...
  const reorderSuggestions = ingredients
    ...
    .map(i => ({
      ...i,
      suggestedQty: Math.max(0, Math.ceil(i.par_level * 2 - i.quantity)),
    }));

  // Static/mock data
  const expiryItems = [
    { name: 'Fresh Milk', days: 2, qty: '5 L' },
    { name: 'Sliced Cheese', days: 3, qty: '2 kg' },
  ];

  const varianceData = [{ name: 'Stock Check', expected: 100, actual: 98, variance: -2 }];
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div>
            ...
            <p className="text-gray-400 text-sm">
              Tracking menu item availability & stock levels
            </p>
          </div>
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-gray-800 max-h-[140px] overflow-y-auto custom-scrollbar">
                {criticalItems.length === 0 && (
                  <p className="p-4 text-sm text-gray-500">Stock levels look good.</p>
                )}
                {criticalItems.map(i => (
                  <div
                    key={i.id}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={cn(
                          ...
                          i.quantity === 0 ? 'bg-red-500' : 'bg-yellow-500'
                        )}
                      />
                      <div>
                        <p className="font-bold text-white text-sm">{i.name}</p>
                        <p className="text-xs text-gray-500">
                          {i.quantity} units remaining
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p
                        className={cn(
                          'text-sm font-bold',
                          i.quantity === 0 ? 'text-red-500' : 'text-yellow-500'
                        )}
                      >
            ...
            <Card className="bg-[#1A1A1A] border-gray-800">
              ...
              <CardContent className="space-y-4">
                {expiryItems.map((item, i) => (
                  <div
                    key={i}
                    className="flex justify-between items-center p-3 rounded-lg bg-orange-500/5 border border-orange-500/10"
                  >
                    <div>
                ...
                <tbody className="divide-y divide-gray-800">
                  {reorderSuggestions.map(item => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-gray-300">
                        ETB{' '}
                        {(
                          item.suggestedQty * item.cost_per_unit
                        ).toLocaleString()}
                      </td>
                  ...
                  {reorderSuggestions.length === 0 && (
                    <tr>
                      ...
                      >
                        No reorders needed. All items &gt; 10 units.
                      </td>
                    </tr>
                  )}
                </tbody>
