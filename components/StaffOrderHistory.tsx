...
import {
    ...
    Hash, User, ShoppingBag, ChevronRight
} from 'lucide-react';

interface StaffOrderHistoryProps {
    orders: Order[];
    view: 'daily' | 'weekly' | 'monthly';
    onOrderClick?: (order: Order) => void;
}

export const StaffOrderHistory: React.FC<StaffOrderHistoryProps> = ({
    orders,
    view,
    onOrderClick
}) => {
    ...
    const groupedOrders = useMemo(() => {
        ...
        orders.forEach(order => {
            ...
            if (view === 'daily') {
                ...
                // Calculate week of year or date range
    ...
    if (orders.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-20 opacity-30 grayscale">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.3em]">No Transactions Recorded</p>
            </div>
        );
    }
                                ...
                                <div className="flex items-center justify-between gap-4">
                                    <div className="flex items-center gap-4 min-w-0">
                                        <div className="w-11 h-11 rounded-xl bg-white/5 flex-shrink-0 flex items-center justify-center border border-white/5 group-hover:bg-primary/10 transition-colors">
                                            <Hash className="w-5 h-5 text-zinc-500 group-hover:text-primary transition-colors" />
                                        </div>
                                        <div className="min-w-0">
                                            <div className="flex items-center gap-2">
                                                <span className="text-[11px] font-black text-white uppercase tracking-tight truncate">
                                                    Order #{order.order_number || order.id.slice(0, 4)}
                                                </span>
